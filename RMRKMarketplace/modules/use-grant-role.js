...
const availableGrantRoleNetworks = {
    'moonbase-alpha': '0x3A854CFc9B912B0219146C4B829cD8Ce43695743',
};
export const useGrantRole = ({ role, account }, _a) => {
    var { chainId, network, signer } = _a, rest = __rest(_a, ["chainId", "network", "signer"]);
    const [receipt, setReceipt] = useState();
    const [waitError, setWaitError] = useState();
    const [isWaiting, setIsWaiting] = useState();
    const prepare = usePrepareContractWrite(Object.assign({ address: availableGrantRoleNetworks[network], chainId, abi: RMRKMarketplace, functionName: 'grantRole', args: [role, account] }, rest));
    ...
    const contract = getContract({
        address: availableGrantRoleNetworks[network],
        abi: RMRKMarketplace,
...
//# sourceMappingURL=use-grant-role.js.map