...
const availableSetAssetForTokenNetworks = {
    'moonbase-alpha': '0x9D2F9635014d9B400C22B30693eed09Bb0585Ef4',
};
export const useSetAssetForToken = ({ collection, tokenId, asset }, _a) => {
    var { chainId, network, signer } = _a, rest = __rest(_a, ["chainId", "network", "signer"]);
    const [receipt, setReceipt] = useState();
    const [waitError, setWaitError] = useState();
    const [isWaiting, setIsWaiting] = useState();
    const prepare = usePrepareContractWrite(Object.assign({ address: availableSetAssetForTokenNetworks[network], chainId, abi: MetaFactoryAbi, functionName: 'setAssetForToken', args: [collection, tokenId, asset] }, rest));
    ...
    const contract = getContract({
        address: availableSetAssetForTokenNetworks[network],
        abi: MetaFactoryAbi,
...
//# sourceMappingURL=use-set-asset-for-token.js.map